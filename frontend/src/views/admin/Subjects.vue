  ...
  <div class="container">
    ...
    >
      ...
      >
        ...
        >
          <tr>
            <th scope="col" class="px-6 py-3">Nome del corso</th>
...
import TableRow from "../../components/admin/subject/SubjectTableRow.vue";
...
const handleButtonSubmit = async (refetchf) => {
  const res = await genericPost(
    `${process.env.VUE_APP_BACKEND_URL}/api/auth/admin/subject`,
    `name=${name.value}`
  );
...
const handleOnDeleteSubmit = async (id, refetchf) => {
  const res = await genericDelete(
    `${process.env.VUE_APP_BACKEND_URL}/api/auth/admin/subject?id=${id}`
  );
  if (res.status === "success") {
    createToast(res.message, { type: "success" });
    refetchf();
